...
| Docker API consumers | `socket-proxy-net` only |
...
The Docker API socket (`/var/run/docker.sock`) grants root-equivalent host access. Direct socket mounts into containers are prohibited in this stack. All Docker API access goes through the Socket Proxy, which blocks:
